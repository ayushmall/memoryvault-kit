...
| `mv init <vault>` | <1s | folder scaffolding, no LLM |
| Connect 1 MCP source (e.g. Granola) | ~2 min | OAuth + permission flow in source's UI |
| `mv ingest` your first batch (50 items) | ~3-5 min | Claude reads each source item, writes a structured memory |
| `mv eval init --from-vault` | <1s | scans your vault, generates ~30 starter questions |
| `mv eval run` | ~5-10s | BM25 over the eval set |
...
| operation | time |
|---|---|
| `mv refresh` (typical morning, ~30 new items) | 2-3 min |
| Answer a question from Claude Code via the kit | <1s (BM25) / ~300ms (with MPS reranker) / ~3s (CPU reranker) |
| `mv daily` (full pipeline: lint + heal + audit + dashboard) | ~30s |
...
|---|---|---|---|
| `mv ingest <granola transcript>` | ~3-8k per item | ~1-2k per item | LLM bill |
| `mv refresh` (50 items, typical day) | ~200-400k total | ~50-100k total | LLM bill |
| `mv ask 'question'` | ~2k (1.5k context + 200 question + 500 reasoning) | ~500-1000 | LLM bill (your Claude Code session) |
| `mv eval init --from-vault` | 0 | 0 | nothing — pure code |
| `mv eval run` | 0 | 0 | nothing — pure BM25 |
| `mv audit / heal / lint` | 0 | 0 | nothing — pure code |
...
- **No LLM in indexing.** The kit's "search engine" is rebuilt on every
  `mv ask` call (it's fast — sub-ms — because the vault is small).
