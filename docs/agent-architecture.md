...
The original `mv-authoring-cycle` skill was a "wake up and do everything"
...
```
   ...
   │                                                              │    
   │  mv-ingest-calendar · mv-ingest-gmail · mv-ingest-slack ·    │    
   │  mv-ingest-notion · mv-ingest-linear · mv-ingest-granola ·  │    
   │  mv-ingest-github-prs · mv-ingest-gdrive                    │    
   │                                                              │    
   ...
   │                                                              │    
   │  mv-heal-agent (alias_map · connect_entities · split_mentions│    
   ...
   │                                                              │    
   │  mv-coverage-agent     — detects 11 classes of gap,         │    
   │                          writes mem_GAP_* memories           │    
   │                                                              │    
   │  mv-queue-router       — reads authoring queue, classifies   │    
   ...
   │ mv-deep-dive │  │ mv-stub-     │  │ mv-          │              
   │              │  │ enricher     │  │ contradiction│              
   │ Native MCP   │  │              │  │ -resolver    │              
   │ call →       │  │ Read Evidence│  │              │              
   ...
   │                                                              │    
   │  mv-eval-runner   — fill_quality + pollution + consistency, │    
   │                     track numbers over time                  │    
   │  mv-quality-judge — Claude-as-judge samples; surfaces        │    
   ...
   │                                                              │    
   │  mv-curator      — dedup; merge same-event memories          │    
   │                    (calendar + granola for same meeting)     │    
   │  mv-traversal    — walks tree for structural insights        │    
...
**Before (mv-authoring-cycle):**
- Single skill handles classification + deep-dive + enrichment + review
- Single cadence (whenever scheduled)
- Single failure point

**After:**
- `mv-queue-router` decides what each queue item needs (pure classification, no doing)
...
|---|---|---|
| `mv-heal-agent` | NEW | The heal chain (was inside `mv migrate`) — pure local; nightly |
| `mv-coverage-agent` | NEW | `coverage_gaps.py` + `enrich_gaps.py` — gap detection + initial enrichment |
| `mv-queue-router` | refactor of `mv-authoring-cycle` | Classify queue items, dispatch — no doing |
| `mv-deep-dive` | NEW | Take a deep-dive task, call the right native MCP, synthesize |
...
A reasonable default schedule (per `mv-schedule`):

| Time | What |
|---|---|
| Hourly during work hours | Pull from active sources (calendar, slack, gmail) |
| Every 4 hours | Heavier ingests (linear, notion) |
| Daily 1 AM | `mv-heal-agent` |
| Daily 2 AM | `mv-coverage-agent` (writes new gaps after heal) |
| Daily 2:30 AM | `mv-queue-router --apply` (drains auto-resolvable) |
| Weekly Mon 3 AM | `mv-eval-runner` (numbers tracked + drift alert) |
| Monthly | `mv-curator`, `mv-quality-judge` |

The `mv-schedule` skill builds this stack on Anthropic Routines (cloud)
or local cron.

## When this matters

Most users won't see the decomposition. They run `mv-setup` once,
`mv-schedule` once, and never invoke a Layer-4 agent by name. The split
shows up in:

- **Debugging**: when something's wrong, you can run a single layer
  manually and inspect (e.g. `mv-heal --verbose` to see what the heal
  pass did)
